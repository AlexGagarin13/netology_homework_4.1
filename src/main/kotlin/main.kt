...
fun main() {
    ...
    val previousTransfersInMonthInPennies1 = 50_000_00.0
    ...
    val transferAmountInPennies3 = 5_000_00.0
...
fun calculateCommission(
    ...
    val commissionToPay: Double = when (accountType) {
        "VK Pay" -> if (previousTransfersInMonthInPennies <= VK_PAY_LIMIT && previousTransfersInMonthInPennies + transferAmountInPennies <= VK_PAY_LIMIT) {
                            0.0
            } else error("Месячный лимит для перевода VK Pay превышен!")
        "MASTERCARD/MAESTRO" -> if (previousTransfersInMonthInPennies <= CARDS_TRANSFER_LIMIT && previousTransfersInMonthInPennies + transferAmountInPennies <= CARDS_TRANSFER_LIMIT) {
        ...
        } else error("Месячный лимит для перевода по картам  превышен!")
        "VISA/MIR" -> if (previousTransfersInMonthInPennies <= CARDS_TRANSFER_LIMIT && previousTransfersInMonthInPennies + transferAmountInPennies <= CARDS_TRANSFER_LIMIT) {
            if (transferAmountInPennies * COMMISSION_VISA_MIR < MINIMUM_COMMISSION_VISA_MIR) {
                MINIMUM_COMMISSION_VISA_MIR
            } else transferAmountInPennies * COMMISSION_VISA_MIR
        } else error("Месячный лимит для перевода по картам превышен!")
        else -> error("Платежная система не поддерживается")
    }
